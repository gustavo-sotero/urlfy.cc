# Plan: Fix Dashboard Zero Metrics

## Problem Statement

The Admin Dashboard displays zero values for:

- **Platform Growth**: "Total de Cliques" = 0, "Novos Usuários" = 0
- **Performance**: "Requisições/segundo" = 0

Root causes:

1. **RPS (Requests/Second)**: The main API handler (`src/app/api/[[...slugs]]/route.ts`) does not call `MetricsService.trackRequest()`, so no request count is accumulated for the RPS calculation job.
2. **Platform Growth**: The database lacks historical data in `analytics_events` and `link_clicks_daily` tables.

---

## Step 1: Instrument API Handler for RPS Tracking
...
2. Add tracking call at the start of `handle()` function (non-blocking):
...
- `MetricsService.trackRequest()` increments a Redis counter (`metrics:req:count`)
- The `rpsCalculationJob` (runs every 60s) reads this counter and computes RPS
- Using `.catch()` ensures tracking failures don't affect request processing

---

## Step 2: Create Analytics Seeder Script

### File: `src/db/scripts/seed-analytics.ts`

**Purpose:** Generate realistic historical data for the last 30 days to populate dashboard charts.
...
async function ensureDemoUser(): Promise<string> {
  ...
  const hashedPassword = await Bun.password.hash('DemoPassword123!');
...
async function createDemoLinks(userId: string): Promise<string[]> {
  ...
  if (existingLinks.length >= SEED_CONFIG.LINK_COUNT) {
    ...
    return existingLinks.map((l) => l.id);
  }
  ...
  return [...existingLinks.map((l) => l.id), ...linkIds];
}
...
function aggregateEventsToDaily(
  ...
  for (const event of events) {
    ...
    if (existing) {
      ...
      // Track unique visitors via Set in a separate structure if needed
    } else {
      dailyMap.set(key, {
        linkId: event.linkId,
        date: dateStr,
        clicks: 1,
        uniqueVisitors: 1 // Simplified: assume 1 unique per event for demo
      });
    }
  }
...
```json
{
  "scripts": {
    // ... existing scripts ...
    "db:seed:analytics": "bun run src/db/scripts/seed-analytics.ts"
  }
}
```

**Location:** After `"db:seed:test-users"` for logical grouping.
...
- [ ] `bun run dev` starts without middleware conflict
- [ ] `bun run db:seed:analytics` completes successfully
- [ ] Dashboard shows non-zero "Total de Cliques"
- [ ] Dashboard shows non-zero "Novos Usuários"
- [ ] Chart displays data points for last 7/30 days
- [ ] Refreshing dashboard multiple times shows RPS > 0 (due to API tracking)

---

## Important Notes

1. **DO NOT** create `src/middleware.ts` - the project uses `src/proxy.ts` (Next.js 16+ pattern)
2. All database operations use Drizzle ORM with proper typing
3. The seeder is idempotent (can be run multiple times safely)
4. Events use `randomUUID()` for IDs (not `nanoid`) to match schema expectations
5. The `link_clicks_daily` table is critical for dashboard performance - raw events are too slow to query
...
| Entity           | Count  | Notes                             |
| ---------------- | ------ | --------------------------------- |
| Users            | 1      | Demo user for analytics           |
| Links            | 20     | Demo short URLs                   |
| Events           | ~3,000 | 20 links × 5 avg clicks × 30 days |
| Daily Aggregates | ~600   | 20 links × 30 days                |

This volume is sufficient to visualize trends without bloating the dev database.
