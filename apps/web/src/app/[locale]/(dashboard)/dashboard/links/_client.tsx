...
import { useLinks, useRestoreLink } from '@/lib/hooks/use-links';

type LinkListView = 'live' | 'deleted';

export function DashboardLinksPageClient() {
  const t = useTranslations('Dashboard');
  const tCommon = useTranslations('Common');
  const [search, setSearch] = useState('');
  const [view, setView] = useState<LinkListView>('live');
  ...
  const restoreLink = useRestoreLink({
    onSuccess: () => toast.success(t('toasts.restoreSuccess')),
    onError: (err) =>
      toast.error(err instanceof Error ? err.message : t('toasts.restoreError'))
  });

  function handleViewChange(nextView: LinkListView) {
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="max-w-2xl text-muted-foreground">
            {showingDeleted ? t('links.deletedSubtitle') : t('links.subtitle')}
          </p>
        </div>
      ...
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex flex-col gap-3 xl:max-w-xl xl:flex-1">
          <div className="flex flex-wrap gap-2">
            <Button
              type="button"
              variant={showingDeleted ? 'outline' : 'default'}
        ...
        data.data.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-border/80 p-8 text-center sm:p-12">
            <p className="text-muted-foreground">
              {search
                ? showingDeleted
                  ? t('empty.noDeletedSearch')
                  : t('empty.noSearch')
                : showingDeleted
                  ? t('empty.noDeletedLinks')
                  : t('empty.noLinks')}
            </p>
            {!search && !showingDeleted && (
          ...
          <div className="space-y-3 sm:space-y-4">
            {data.data.map((link) => (
              <LinkCard
                ...
                mode={showingDeleted ? 'deleted' : 'live'}
