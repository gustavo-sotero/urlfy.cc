...
export const RATE_LIMITS = {
  ...
  // Admin Endpoints (NEW)
  ADMIN_GENERAL: { windowMs: 60 * 1000, max: 30 }, // 30/min for general admin actions
  ADMIN_USER_MANAGEMENT: { windowMs: 60 * 1000, max: 20 }, // 20/min for user ban/unban/role changes
  ADMIN_LINK_BAN: { windowMs: 60 * 1000, max: 50 }, // 50/min for link banning (can be frequent for abuse)
  ADMIN_BULK_ACTIONS: { windowMs: 60 * 1000, max: 10 }, // 10/min for bulk operations

  // Authentication
  AUTH_SIGN_IN: { windowMs: 15 * 60 * 1000, max: 5 }, // 5/15min (brute force)
  AUTH_SIGN_UP: { windowMs: 60 * 60 * 1000, max: 3 }, // 3/hour
