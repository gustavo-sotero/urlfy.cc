...
interface ClickEventStream {
  /** Stable logical event id generated at ingress and preserved across retries. */
  eventId?: string;
  linkId: string;
  shortCode: string;
...
class AnalyticsClickWorker extends WorkerBase<ClickEventStream> {
  ...
  protected async processMessage(
    ...
    const streamMessageId = this.getLogicalEventId(id, payload);
    const enriched = await this.enrichClickEvent(streamMessageId, payload);
    ...
    const linkClickCounts = await db.transaction(async (tx) => {
      const inserted = await tx
        ...
        .values(this.mapEnrichedToRow(enriched, streamMessageId))
        .onConflictDoNothing()
  ...
  protected override async processMessages(
    ...
    const enrichResults = await Promise.allSettled(
      messages.map(async (msg) => {
        const streamMessageId = this.getLogicalEventId(msg.id, msg.data);

        return {
          message: msg,
          streamMessageId,
          enriched: await this.enrichClickEvent(streamMessageId, msg.data)
        };
      })
    );
      ...
      const { insertedStreamIds, linkClickCounts } = await db.transaction(
        async (tx) => {
          ...
          const insertedRows = await tx
            .insert(analyticsEvents)
            .values(
              enrichedEvents.map(({ enriched, streamMessageId }) =>
                this.mapEnrichedToRow(enriched, streamMessageId)
              )
            )
            .onConflictDoNothing()
            .returning({
   ...
   * streamMessageId is a logical event id generated by the producer and
   * preserved by retries. Redis message ids are only a fallback for legacy
   * in-flight entries that predate eventId.
   */
  ...
  private buildLinkClickCounts(
    ...
    for (const { enriched, streamMessageId } of enrichedEvents) {
      if (!insertedStreamIds.has(streamMessageId)) {
        this.logger.debug('[AnalyticsClickWorker] Duplicate event skipped', {
          streamMessageId
        });
        continue;
  ...
  private getLogicalEventId(
    messageId: string,
    payload: ClickEventStream
  ): string {
    const rawPayload = payload as ClickEventStream & {
      originalId?: unknown;
    };

    if (typeof rawPayload.eventId === 'string' && rawPayload.eventId) {
      return rawPayload.eventId;
    }

    if (typeof rawPayload.originalId === 'string' && rawPayload.originalId) {
      return rawPayload.originalId;
    }
