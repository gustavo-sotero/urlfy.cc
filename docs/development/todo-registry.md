...
- **Changes delivered:**
  ...
  6. **Browser error reporting** — `apps/web/src/lib/browser-logger.ts` and `apps/web/src/lib/browser-log-contract.ts` now define the canonical client payload; key dashboard/admin/form surfaces and `/ops/monitor/log` all use the same requestId/context-aware contract.
...
- **Changes delivered:**
  ...
  3. **Monitor log rate limiting** — `/ops/monitor/log` switched from spoofable in-memory rate limiter to distributed Redis-backed rate limiting via canonical `rateLimiter` and `getClientIp()`.
